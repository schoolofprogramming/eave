...
class VoiceStateUpdateHandler {
  ...
  handle(oldState: VoiceState, newState: VoiceState) {
    ...
    console.log(
      `Members in ${oldState.channel?.name} (old): ${oldMemberCount}`
    );
    console.log(
      `Members in ${newState.channel?.name} (new): ${newMemberCount}`
    );
    ...
    console.log(
      `${newState.member?.displayName} ${change}: ${oldState.channel?.name} -> ${newState.channel?.name}`
    );
    ...
    const seconds = (
      newMemberCount >= MIN_MEMBER_COUNT
        ? this.voiceActivity.start
        : oldMemberCount < MIN_MEMBER_COUNT
        ? this.voiceActivity.stop
        : () => null
    ).bind(this.voiceActivity)(); // XXX
    ...
    if (seconds == null || seconds < MIN_VC_SESSION_DURATION) return;
